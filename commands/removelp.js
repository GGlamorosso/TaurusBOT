...
export const data = new SlashCommandBuilder()
  .setName('removelp')
  .setDescription('Retirer des LP à un utilisateur.')
  .addUserOption(opt => opt.setName('utilisateur').setDescription('Utilisateur cible').setRequired(true))
  .addIntegerOption(opt => opt.setName('quantité').setDescription('Quantité de LP à retirer').setRequired(true))
...
export async function execute(interaction, client, context) {
  ...
  data.lp = Math.max(0, (data.lp ?? 0) - amount);
  saveSoon();
  const guildMember = await interaction.guild.members.fetch(target.id).catch(() => null);
  if (guildMember) {
    await applyRankRole(guildMember, data.lp);
  }
  await interaction.editReply({ content: `✅ **${amount} LP** retirés de **${target.tag}** (total: ${data.lp}).` });
  await logToChannel(client, `➖ ${interaction.user.tag} a retiré ${amount} LP de ${target.tag} (total ${data.lp}).`);
}
